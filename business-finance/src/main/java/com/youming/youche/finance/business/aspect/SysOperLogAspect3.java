package com.youming.youche.finance.business.aspect;
...
public class SysOperLogAspect3 {
	...
	private final Logger logger = LoggerFactory.getLogger(SysOperLogAspect3.class);
	...
	public void saveSysLog(JoinPoint joinPoint, Object rvt) {
		 ...
		 *
		 * ua.getBrowser().toString();//Chrome ua.getVersion();//14.0.835.163
		 * ua.getEngine().toString();//Webkit ua.getEngineVersion();//535.1
		 * ua.getOs().toString();//Windows 7 ua.getPlatform().toString();//Windows
		 */
