...
public class OilEtcTurnCash {
    ...
    public List dealToOrder(ParametersNewDto inParam, List<BusiSubjectsRel> rels, LoginInfo user) {
        ...
        Long batchId = Long.valueOf(inParam.getBatchId());
        ...
        for (BusiSubjectsRel rel : rels) {
            ...
            //油转现(油支出)
            if (rel.getSubjectsId() == EnumConsts.SubjectIds.OIL_SURPLUS) {
                // 资金流水操作
                ...
                off.setBusiTable("OIL_ETC_TURN_CASH");// 业务对象表
                off.setBusiKey(batchId);// 业务流水ID
            ...
            //油转现(抵扣借支油)
            if (rel.getSubjectsId() == EnumConsts.SubjectIds.OIL_SURPLUS_DEDUCTIBLE_LOAN) {
                // 资金流水操作
                OrderFundFlow off = new OrderFundFlow();
                off.setAmount(rel.getAmountFee());// 交易金额（单位分）
                off.setOrderId(olTemp.getOrderId());// 订单ID
            ...
            //油转现(现金收入)
            if (rel.getSubjectsId() == EnumConsts.SubjectIds.OIL_SURPLUS_TRUN_CASH) {
                // 资金流水操作
                OrderFundFlow off = new OrderFundFlow();
                off.setAmount(rel.getAmountFee());// 交易金额（单位分）
                off.setOrderId(olTemp.getOrderId());// 订单ID
            ...
            //ETC转现(ETC支出)
            if (rel.getSubjectsId() == EnumConsts.SubjectIds.ETC_SURPLUS) {
                // 资金流水操作
                OrderFundFlow off = new OrderFundFlow();
                off.setAmount(rel.getAmountFee());// 交易金额（单位分）
                off.setOrderId(olTemp.getOrderId());// 订单ID
            ...
            //ETC转现(现金收入)
            if (rel.getSubjectsId() == EnumConsts.SubjectIds.ETC_SURPLUS_TRUN_CASH) {
                // 资金流水操作
                OrderFundFlow off = new OrderFundFlow();
                off.setAmount(rel.getAmountFee());// 交易金额（单位分）
                off.setOrderId(olTemp.getOrderId());// 订单ID
            ...
            //抵扣欠款
            if (rel.getSubjectsId() == EnumConsts.SubjectIds.ETC_SURPLUS_TRUN_NOTDUE ||
                    rel.getSubjectsId() == EnumConsts.SubjectIds.OIL_SURPLUS_TRUN_NOTDUE) {
                // 资金流水操作
                OrderFundFlow off = new OrderFundFlow();
                off.setAmount(rel.getAmountFee());// 交易金额（单位分）
                off.setOrderId(olTemp.getOrderId());// 订单ID
                ...
                for (OrderLimit olTempFinal : orderLimitsFinal) {
                    if (olTempFinal.getMatchAmount() != null && olTempFinal.getMatchAmount() > 0L) {
                        this.payForDebt(off.getOrderId(),olTempFinal, rel, batchId, inParam, user);
                    }
                }
            }
     ...
     * 抵扣欠款流水处理
     */
    public void payForDebt(long orderId, OrderLimit ol, BusiSubjectsRel rel, Long flowId, ParametersNewDto inParam, LoginInfo user){
        //资金流水操作
        OrderFundFlow off = new OrderFundFlow();
