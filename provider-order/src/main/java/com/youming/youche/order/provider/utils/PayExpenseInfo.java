...
public class PayExpenseInfo {
    ...
    public List dealToOrderNew(ParametersNewDto inParam, List<BusiSubjectsRel> rels,LoginInfo user)  {
        ...
        //查询订单限制数据
        List<OrderLimit> orderLimits = orderLimitService.getOrderLimitUserId(userId, orderId, -1L,-1,user);
        ...
        for (BusiSubjectsRel rel : rels) {
            ...
            if (rel.getSubjectsId() == EnumConsts.SubjectIds.EXPENSE_RECEIVABLE) {
//    			ol.setNoPayCash(ol.getNoPayCash() + rel.getAmountFee());
//    			ol.setNoWithdrawCash(ol.getNoWithdrawCash() +  rel.getAmountFee());
                ol.setExpenseFee(ol.getExpenseFee() == null ? rel.getAmountFee() : ol.getExpenseFee() + rel.getAmountFee());//记录报销金额

                off.setInoutSts("in");//收支状态:收in支out转io
                orderLimitService.saveOrUpdate(ol);
            } else if (rel.getSubjectsId() == EnumConsts.SubjectIds.EXPENSE_HANDLE) {
